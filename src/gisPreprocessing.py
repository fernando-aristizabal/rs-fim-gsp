...
def gisPreprocessing():

	# extract ground control points
	print('.... Extracting Ground Control Points from Original SAR rasters ....')
	call(['./scripts/functions/extractGCPs.py','data/texas/sar/processed/gcp.txt','data/sar/S1A_IW_GRDH_1SDV_20161012T111514_20161012T111543_013456_01580C_1783.SAFE/measurement/s1a-iw-grd-vv-20161012t111514-20161012t111543-013456-01580c-001.tiff'])

	# Re-add ground control points
	print('.... Adding Ground Control Points To SAR Rasters ....')
	call(['./scripts/functions/addGCPs.py','data/texas/sar/processed/gcp.txt','data/sar/processed/vv_cal_spk.tif','data/sar/processed/vv_cal_spk_gcp.tif'])
	call(['./scripts/functions/addGCPs.py','data/texas/sar/processed/gcp.txt','data/sar/processed/vh_cal_spk.tif','data/sar/processed/vh_cal_spk_gcp.tif'])

	# Mosaic HAND and catchments
        #call(['gdalbuildvrt', 'data/texas/hand/processed/hand.vrt','data/texas/hand/original/*hand.tif'])
        #call(['gdalbuildvrt', 'data/texas/hand/processed/catchmask.vrt','data/texas/hand/original/*catchmask.tif'])

        # Reproject HAND and catchments to Cartesian
        for i in *catchmask.tif; do echo $i; gdalwarp -t_srs cartesianProjection $i ../processed/${i%.*f}_proj.tif ; done

        # Merge
        read fsize ncols nrows ndv xmin ymin xmax ymax cellsize_resx cellsize_resy <<< $(./scripts/functions/getRasterInfoNative.py data/texas/hand/processed/120903hand_proj.tif)
        gdal_merge.py -v -o data/texas/hand/processed/hand_merged.tif data/texas/hand/processed/*hand_proj.tif
        gdal_translate -a_nodata $ndv data/texas/hand/processed/hand_merged.tif data/texas/hand/processed/hand_merged_ndv.tif
        
        # Rescale
        gdalwarp -overwrite -r bilinear -multi -co "BIGTIFF=YES" -tr $cellsize_resx $cellsize_resy data/texas/hand/processed/hand_merged_ndv.tif data/texas/hand/processed/hand_merged_ndv_scaled.tif
        gdalwarp -overwrite -multi -co "BIGTIFF=YES" -tr $cellsize_resx $cellsize_resy data/texas/hand/processed/catchmask_merged_ndv.tif data/texas/hand/processed/catchmask_merged_ndv_scaled.tif

        # Clip
        gdalwarp -overwrite -r bilinear -multi -co "BIGTIFF=YES" -te $xmin $ymin $xmax $ymax data/texas/hand/processed/hand_merged_ndv_scaled.tif data/texas/hand/processed/hand_merged_ndv_scaled_clipped.tif
        gdalwarp -overwrite -multi -co "BIGTIFF=YES" -te $xmin $ymin $xmax $ymax data/texas/hand/processed/catchmask_merged_ndv_scaled.tif data/texas/hand/processed/catchmask_merged_ndv_scaled_clipped.tif
	...
	os.system('gdalwarp -r bilinear -ot VRT -t_srs \'{}\' -overwrite data/sar/processed/vv_cal_spk_gcp.tif data/sar/processed/vv_cal_spk_gcp_proj.vrt'.format(sphericalProjection))
	os.remove('data/sar/processed/vv_cal_spk_gcp.tif')
	os.system('gdalwarp -r bilinear -ot VRT -t_srs \'{}\' -overwrite data/sar/processed/vh_cal_spk_gcp.tif data/sar/processed/vh_cal_spk_gcp_proj.vrt'.format(sphericalProjection))
	os.remove('data/sar/processed/vh_cal_spk_gcp.tif')
